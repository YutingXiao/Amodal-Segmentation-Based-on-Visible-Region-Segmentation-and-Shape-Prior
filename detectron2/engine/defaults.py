...
from detectron2.evaluation import (
    ...
    embedding_inference_on_train_dataset,
    print_csv_format,
    verify_results,
)
...
from detectron2.data.dataset_mapper import AmodalDatasetMapper
...
class DefaultTrainer(SimpleTrainer):
    ...
    def __init__(self, cfg):
        ...
        if self.apply_mul_opts:
            ...
            self.scheduler_recon = self.build_lr_scheduler(cfg, optimizer_recon)
            ...
            optimizer = self.build_optimizer(cfg, model, ty_opt=cfg.SOLVER.OPT_TYPE)
    ...
    def build_train_loader(cls, cfg):
        ...
        if cfg.DATALOADER.MAPPER == "amodal_and_visible":
            return build_detection_train_loader(cfg, mapper=AmodalDatasetMapper(cfg, is_train=True))
    ...
    def build_test_loader(cls, cfg, dataset_name):
        ...
        """
        if cfg.DATALOADER.MAPPER == "amodal_and_visible":
            return build_detection_test_loader(cfg, mapper=AmodalDatasetMapper(cfg, is_train=False))
    ...
    def test(cls, cfg, model, evaluators=None):
        ...
        if (not cfg.MODEL.ROI_MASK_HEAD.RECON_NET.LOAD_CODEBOOK or not os.path.exists("{}_codebook.npy".format(dataset))) \
            ...
            torch.save(model.roi_heads.recon_net.state_dict(), "{}/{}_recon_net.pth".format(model._cfg.OUTPUT_DIR, dataset))
            logger.info("Recon net saved")
            np.save('{}/{}_codebook.npy'.format(model._cfg.OUTPUT_DIR, dataset), model.roi_heads.recon_net.vector_dict)
