...
def load_cocoa_json(json_file, image_root, dataset_name=None):
    """
    ...
    with contextlib.redirect_stdout(io.StringIO()):
        cocoa_api = COCO(json_file)
    if timer.seconds() > 1:
        logger.info("Loading {} takes {:.2f} seconds.".format(json_file, timer.seconds()))
    id_map = None
    if dataset_name is not None:
        meta = MetadataCatalog.get(dataset_name)
        cat_ids = sorted(cocoa_api.getCatIds())
        cats = cocoa_api.loadCats(cat_ids)
...
"""
    ...
    img_ids = sorted(list(cocoa_api.imgs.keys()))
    ...
    #  'id': 1268}
    imgs = cocoa_api.loadImgs(img_ids)
    ...
    #   'id': 42986},
    #  ...]
    anns = [cocoa_api.imgToAnns[img_id] for img_id in img_ids]
        ...
        for anno in anno_dict_list:
            ...
            if segm:  # either list[list[float]] or dict(RLE)
                ...
                if not isinstance(segm, dict):
                    ...
                    segm = [segm] if len(segm) != 1 else segm
                ...
                if not isinstance(vis_segm, dict):
                    # filter out invalid polygons (< 3 points)
                    vis_segm = [vis_segm] if len(vis_segm) != 1 else vis_segm
