...
__all__ = ["load_coco_json", "load_sem_seg", "check_anno"]
...
def load_coco_json(json_file, image_root, dataset_name=None, extra_annotation_keys=None):
    ...
    if dataset_name is not None:
        ...
        if not (min(cat_ids) == 1 and max(cat_ids) == len(cat_ids)):
            if "sailvos" not in dataset_name:
                logger.warning(
                    """
...
def check_anno(dataset_dicts):
    ...
    for item in dataset_dicts:
        ...
        if len(item['annotations']) == 0:
            ...
            for ob in item['annotations']:
                if isinstance(ob['segmentation'], dict):
                    print(dataset_dicts.index(item))
                    label = 1
            j = j + 1 if label == 1 else j
    print('all no anno:{}, all dict:{}'.format(i, j))
...
if __name__ == "__main__":
    ...
    dicts = load_coco_amodal_json(sys.argv[1], sys.argv[2], sys.argv[3])
