...
class COCOEvaluator(DatasetEvaluator):
        ...
        if dataset_name.endswith("visible"):
            ...
            if dataset_name.find('d2sa') != -1:
                for key, _ in self._coco_api.anns.items():
                    self._coco_api.anns[key]["segmentation"] = self._coco_api.anns[key]["visible_mask"]
            elif dataset_name.find("kins") != -1:
                for key, _ in self._coco_api.anns.items():
                    self._coco_api.anns[key]["segmentation"] = self._coco_api.anns[key]["inmodal_seg"]
    ...
    def visualization(self, inputs, outputs):
        ...
        # img_gt = inputs[0]["image"].cpu().numpy()
        # img_gt = img_gt[::-1, :, :]  # if self.input_format == "BGR":
        # img_gt = img_gt.transpose(1, 2, 0)
        # v_gt = Visualizer(img_gt, None)
        # v_gt = v_gt.overlay_instances(boxes=inputs[0]["instances"].gt_boxes,
        #                               masks=inputs[0]["instances"].gt_masks)
        # anno_img = v_gt.get_image()
        # vis_image = anno_img.transpose(2, 0, 1)
        # vis_name = "Val: 1. GT"
        # storage.put_image(vis_name, vis_image)

        img = np.array(Image.open(inputs[0]['file_name']))
        v_pred = Visualizer(img, None)
        box_size = min(len(outputs[0]['instances'].pred_boxes), vis_box_max)
        v_pred = v_pred.overlay_instances(boxes=outputs[0]["instances"].pred_boxes[0:box_size].tensor.cpu().numpy(),
                                          masks=outputs[0]["instances"].pred_masks[0:box_size].cpu().numpy())
        pred_img = v_pred.get_image()
        # vis_image = np.concatenate((anno_img, pred_img), axis=1)
        vis_image = pred_img.transpose(2, 0, 1)
        vis_name = "Val: Predicted masks"
        storage.put_image(vis_name, vis_image)
    ...
    def _eval_predictions(self, tasks):
        ...
        for task in sorted(tasks):
            # if task == "segm":
                # logging.info("using visible segmentation")
                # for key, ann in self._coco_api.anns.items():
                #     self._coco_api.anns[key]["segmentation"] = self._coco_api.anns[key]["visible_mask"]
